...
impl DatabaseSettings {
 ...
 pub fn with_db (&self) -> PgConnectOptions {
  ...
  PgConnectOptions::new().host(&self.host).username(&self.username).password(self.password.expose_secret()).database(&self.database_name).port(self.port).ssl_mode(ssl_mode)
 }
}
